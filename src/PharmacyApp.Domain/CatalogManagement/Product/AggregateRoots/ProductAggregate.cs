...
public class ProductAggregate : AggregateRoot<Guid>
{
    ...
    private ProductAggregate(Guid id, string name, ProductDescription description, Money price, int stockQuantity, bool isCosmetic, CategoryId categoryId)
        : base(id)
    {
        ProductName = name;
    ...
    public static ProductAggregate Create(string name, ProductDescription description, Money price, int stockQuantity, bool isCosmetic, CategoryId categoryId)
    {
        if (string.IsNullOrWhiteSpace(name)) throw new ArgumentException("Product name cannot be empty.");
        if (price.Amount < 0) throw new ArgumentException("Product price cannot be negative.");
        if (stockQuantity < 0) throw new ArgumentException("Product stock cannot be negative.");
